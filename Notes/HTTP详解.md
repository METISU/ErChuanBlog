...
1. 客户端发送`Client Hello`消息，并带上客户端支持的密码套件A、随机数（用于生成对称密码使用）、支持的压缩方法
2. 服务端收到`Client Hello`消息后，返回`Server Hello`消息，附带选择的密码套件、随机数B（用于生成对称密码使用）、证书
3. 客户端逐级验证证书是否有效
4. 证书验证完毕，客户端生成第三个随机数`pre-master key`，并用服务端的公钥加密，发送给服务端。通知通知编码方式改变，之后都用三个随机数（随机数A、随机数B、pre-master key）生成的秘钥进行加密通讯，通知通知握手结束，附带前面内容的hash值。
5. 服务端发送编码方式改变通知，通知之后使用生成的秘钥加密。同时通知握手结束，发送前面内容hash值，供客户端校验。
6. 开始对称加密通信。
